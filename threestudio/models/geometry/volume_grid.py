...
class VolumeGrid(BaseImplicitGeometry):
        ...
        if self.cfg.density_bias == "blob":
            ...
            density_bias: Float[Tensor, "*N 1"] = \
                self.cfg.density_blob_scale * (
                    1 - torch.sqrt((points.detach() ** 2).sum(dim=-1)) / self.cfg.density_blob_std)[..., None]
            return density_bias
    ...
    @staticmethod
    def get_trilinear_feature(points, grid):
        """ 根据grid获得points的三线性插值特征.
        :param points: (..., input_dims).
        :param grid: (1, n_feat_dims, g1, g2, g3).
        :return (..., n_feat_dims).
        """
        assert points.shape[-1] == 3
        n_feat_dims = grid.shape[1]
        # 1. Grid采样. (1, n_feat_dims, 1, 1, N).
        """ @形状 points: (1, 1, 1, N, 3), grid: (1, n_feat_dims, g1, g2, g3). """
        out = F.grid_sample(grid, points.view(1, 1, 1, -1, 3), align_corners=False, mode="bilinear")
        # 2. Reshape. (n_feat_dims, N) -> (N, n_feat_dims) -> (..., n_feat_dims).
        out = out.reshape(n_feat_dims, -1).T.reshape(*points.shape[:-1], n_feat_dims)
        return out

    def forward(self, points: Float[Tensor, "*N Di"], output_normal=False) -> Dict[str, Float[Tensor, "..."]]:
        ...
        # breakpoint()
        density = get_activation(self.cfg.density_activation)(density + self.get_density_bias(points_unscaled))
        output = {"density": density, "features": features}
        ...
        if output_normal:
            if self.cfg.normal_type == "finite_difference" or self.cfg.normal_type == "finite_difference_laplacian":
                ...
                if self.cfg.normal_type == "finite_difference_laplacian":
                    ...
                    density_offset = self.forward_density(points_offset)
                    normal = -0.5 * (density_offset[..., 0::2, 0] - density_offset[..., 1::2, 0]) / eps
                else:
                    offsets = torch.as_tensor([[eps, 0.0, 0.0], [0.0, eps, 0.0], [0.0, 0.0, eps]]).to(points_unscaled)
                    points_offset = (points_unscaled[..., None, :] + offsets).clamp(-self.cfg.radius, self.cfg.radius)
                    density_offset = self.forward_density(points_offset)
    ...
    def export(self, points: Float[Tensor, "*N Di"], **kwargs) -> Dict[str, Any]:
        out: Dict[str, Any] = {}
        if self.cfg.n_feature_dims == 0: return out
