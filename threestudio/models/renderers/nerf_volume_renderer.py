...
class NeRFVolumeRenderer(VolumeRenderer):
    ...
    def configure(self, geometry: BaseImplicitGeometry, material: BaseMaterial, background: BaseBackground) -> None:
        ...
        if self.cfg.estimator == "occgrid":
            ...
            self.prop_scheduler = parse_scheduler_to_instance(self.cfg.prop_scheduler_config, self.prop_optim) \
                if self.cfg.prop_scheduler_config is not None else None
    ...
    def forward(self, rays_o: Float[Tensor, "B H W 3"], rays_d: Float[Tensor, "B H W 3"],
        ...
        """
        :param rays_o: (batch, H, W, 3). 光线源点(相机位置)[世界坐标系].
        :param rays_d: (batch, H, W, 3). 每个像素的光线方向[世界坐标系].
        :param light_positions: (batch, 3). 光线位置. 例如，camera_positions归一化向量乘上一个模长.
        :param bg_color: None. 背景颜色？
        :param kwargs:
            - mvp_matrix: (batch, 4, 4). MPV矩阵：从相机坐标系到成像平面的变换矩阵.
            - proj_mtx: (batch, 4, 4). 投影矩阵：将世界坐标系中的点变换到成像平面上.
            - camera_positions: (batch, 3). 相机位置[世界坐标系].
            - c2w: (batch, 4, 4). 相机坐标系到世界坐标系的变换矩阵.
            - elevation: (batch, ). 相机的elevation.
            - azimuth: (batch, ). 相机的azimuth.
            - camera_distances: (batch, ). 相机距离(相机位置到世界中心的距离).
            - height/width: Int. 渲染图像长/宽.
            - fovy: (batch, ). 相机视场角.
        """
        ...
        if self.cfg.estimator == "occgrid":
            if not self.cfg.grid_prune:
                with torch.no_grad():
                    ray_indices, t_starts_, t_ends_ = self.estimator.sampling(
                        rays_o_flatten, rays_d_flatten, sigma_fn=None, near_plane=self.cfg.near_plane, far_plane=self.cfg.far_plane,
                        render_step_size=self.render_step_size, alpha_thre=0.0, stratified=self.randomized, cone_angle=0.0,
                        early_stop_eps=0)
            else:
                """ sigma_fn
                @说明：计算每个样本的nerf密度. 估计器（如nerfacc.OccGridEstimator）会使用这个函数来发现表面（surface）.
                """
                ...
                def sigma_fn(t_starts, t_ends, ray_indices):
                    ...
                    if self.training:
                        ...
                        sigma = chunk_batch(
                            self.geometry.forward_density, self.cfg.eval_chunk_size, positions)[..., 0]
                    return sigma

                with torch.no_grad():
                    ray_indices, t_starts_, t_ends_ = self.estimator.sampling(
                        rays_o_flatten, rays_d_flatten, sigma_fn=sigma_fn if self.cfg.prune_alpha_threshold else None,
                        near_plane=self.cfg.near_plane, far_plane=self.cfg.far_plane,
                        render_step_size=self.render_step_size, alpha_thre=0.01 if self.cfg.prune_alpha_threshold else 0.0,
                        stratified=self.randomized, cone_angle=0.0)
        elif self.cfg.estimator == "proposal":
            ...
            def prop_sigma_fn(t_starts: Float[Tensor, "Nr Ns"], t_ends: Float[Tensor, "Nr Ns"], proposal_network):
                ...
                density_before_activation = proposal_network(
                    positions.view(-1, 3)).view(*positions.shape[:-1], 1).to(positions)
            ...
            t_starts_, t_ends_ = self.estimator.sampling(
                ...
                prop_samples=[self.cfg.num_samples_per_ray_proposal], num_samples=self.cfg.num_samples_per_ray,
                n_rays=n_rays, near_plane=self.cfg.near_plane, far_plane=self.cfg.far_plane, sampling_type="uniform",
                stratified=self.randomized, requires_grad=self.vars_in_forward["requires_grad"])
            ...
            t_starts_, t_ends_ = self.estimator.sampling(
                prop_sigma_fns=[partial(prop_sigma_fn, proposal_network=self.geometry)], prop_samples=[self.cfg.num_samples_per_ray_importance],
                num_samples=self.cfg.num_samples_per_ray, n_rays=n_rays, near_plane=self.cfg.near_plane,
                far_plane=self.cfg.far_plane, sampling_type="uniform", stratified=self.randomized)
            ray_indices = torch.arange(n_rays, device=rays_o_flatten.device).unsqueeze(-1).expand(-1, t_starts_.shape[1])
            ray_indices = ray_indices.flatten()
            t_starts_ = t_starts_.flatten()
            t_ends_ = t_ends_.flatten()
        ...
        """ 所有渲染可见位置的光线interval的数据. 这里的validate_empty_rays只不过检查是不是空的，即n_visible_intervals是否为零.
        @shape: (n_visible_intervals, ).
        """
        ray_indices, t_starts_, t_ends_ = validate_empty_rays(ray_indices, t_starts_, t_ends_)

        ################################################################################################################
        """ 获取如下数据：
        @positions: (n_visible_intervals, ). 可见光线intervals的中心，也就是采样点.
        @t_light_positions.
        """
        ################################################################################################################
        ...
        if self.training:
            """ Geometry（这里是ImplicitVolume）的输出.
            - density: (n_visible_intervals, 1).
            - features: (n_visible_intervals, feat_channels).
            - normal: (n_visible_intervals, 3).
            - shading_normal: (n_visible_intervals, 3).
            """
            ...
            geo_out = chunk_batch(
                self.geometry, self.cfg.eval_chunk_size, positions, output_normal=self.material.requires_normal)
            rgb_fg_all = chunk_batch(
                self.material, self.cfg.eval_chunk_size, viewdirs=t_dirs, positions=positions,
                light_positions=t_light_positions, **geo_out)
            comp_rgb_bg = chunk_batch(self.background, self.cfg.eval_chunk_size, dirs=rays_d)

        # --------------------------------------------------------------------------------------------------------------
        """ 计算weights和transmittance.
        @shape: (n_visible_intervals, ). 每一个光线interval对应的weight和透过率.
        """
        # --------------------------------------------------------------------------------------------------------------
        weights: Float[Tensor, "Nr 1"]
        weights_, trans_, _ = nerfacc.render_weight_from_density(
            t_starts[..., 0], t_ends[..., 0], geo_out["density"][..., 0], ray_indices=ray_indices, n_rays=n_rays)
        if self.training and self.cfg.estimator == "proposal":
            self.vars_in_forward["trans"] = trans_.reshape(n_rays, -1)

        ################################################################################################################
        """ 叠加每个光线interval结果来获得以下结果：
        @opacity: (n_rays, 1). 不透明度.
        @depth: (n_rays, 1). 深度.
        @comp_rgb_fg: (n_rays, 3). 渲染前景颜色.
        """
        ################################################################################################################
        ...
        # --------------------------------------------------------------------------------------------------------------
        """ 其它：z_mean / z_variance_unmasked / z_variance. (n_rays, 1). """
        # --------------------------------------------------------------------------------------------------------------
        ...
        z_variance_unmasked = nerfacc.accumulate_along_rays(
            weights_normalized[..., 0], values=(t_positions - z_mean[ray_indices]) ** 2, ray_indices=ray_indices, n_rays=n_rays)
        ...
        if self.training:
            ...
            if "normal" in geo_out:
                if self.cfg.return_comp_normal:
                    ...
                    out.update({"comp_normal": comp_normal.view(batch_size, height, width, 3)})
                if self.cfg.return_normal_perturb:
                    normal_perturb = self.geometry(
                        positions + torch.randn_like(positions) * 1e-2, output_normal=self.material.requires_normal,
                    )["normal"]
    ...
    def update_step_end(self, epoch: int, global_step: int) -> None:
        if self.cfg.estimator == "proposal" and self.training:
            self.estimator.update_every_n_steps(
                self.vars_in_forward["trans"], self.vars_in_forward["requires_grad"], loss_scaler=1.0)
