...
class ImplicitSDF(BaseImplicitGeometry):
    ...
    def initialize_shape(self) -> None:
        ...
        if self.cfg.sdf_bias != 0.0:
            threestudio.warn("shape_init and sdf_bias are both specified, which may lead to unexpected results.")

        ################################################################################################################
        # 计算指定点处的SDF值. (n_points, 3) => (n_points, 1).
        ################################################################################################################
        ...
        for _ in tqdm(range(1000), desc=f"Initializing SDF to a(n) {self.cfg.shape_init}:", disable=get_rank() != 0):
            points_rand = (torch.rand((10000, 3), dtype=torch.float32).to(self.device) * 2.0 - 1.0)
    ...
    def get_shifted_sdf(self, points: Float[Tensor, "*N Di"], sdf: Float[Tensor, "*N 1"]) -> Float[Tensor, "*N 1"]:
        """ 在sdf的基础上施加bias，得到最后的sdf.
        :param points: (..., n_input_dims).
        :param sdf: (..., 1).
        :return (..., 1).
        """
    ...
    def forward(self, points: Float[Tensor, "*N Di"], output_normal: bool = False) -> Dict[str, Float[Tensor, "..."]]:
        ...
        points_unscaled, points = points, contract_to_unisphere(points, self.bbox, self.unbounded)  # points normalized to (0, 1)
    ...
    def forward_sdf(self, points: Float[Tensor, "*N Di"]) -> Float[Tensor, "*N 1"]:
        ...
        """
        """ 归一化点. (..., n_input_dims). """
        points_unscaled, points = points, contract_to_unisphere(points, self.bbox, self.unbounded)
        # 1. 预测SDF. (..., 1).
    ...
    def forward_field(self, points: Float[Tensor, "*N Di"]) -> Tuple[Float[Tensor, "*N 1"], Optional[Float[Tensor, "*N 3"]]]:
        ...
        :return
            - sdf: 输入points的场值. (..., 1).
            - deformation: 输入points的形变. (..., 3).
        """
        """ 归一化输入点. (..., n_input_dims). """
        points_unscaled, points = points, contract_to_unisphere(points, self.bbox, self.unbounded)
        # 1. Encoding. (..., n_hidden_dims).
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        if self.cfg.normal_type == "finite_difference" or self.cfg.normal_type == "finite_difference_laplacian":
            if isinstance(self.cfg.finite_difference_normal_eps, float):
                ...
                assert hg_conf.otype == "ProgressiveBandHashGrid", "finite_difference_normal_eps=progressive only works with ProgressiveBandHashGrid"
                current_level = min(hg_conf.start_level + max(global_step - hg_conf.start_step, 0) // hg_conf.update_steps, hg_conf.n_levels)
                ...
                if grid_size != self.finite_difference_normal_eps:
                    threestudio.info(f"Update finite_difference_normal_eps to {grid_size}")
