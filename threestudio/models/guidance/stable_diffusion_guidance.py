...
class StableDiffusionGuidance(BaseObject):
    ...
    def compute_grad_sjc(self, latents: Float[Tensor, "B 4 64 64"], t: Int[Tensor, "B"], prompt_utils: PromptProcessorOutput,
        ...
        if prompt_utils.use_perp_neg:
            ...
            with torch.no_grad():
                ...
                noise_pred = self.forward_unet(
                    latent_model_input, torch.cat([t] * 4), encoder_hidden_states=text_embeddings)  # (4B, 3, 64, 64)
        ...
        if self.cfg.var_red: grad = -(Ds - y) / sigma
        else: grad = -(Ds - zs) / sigma
