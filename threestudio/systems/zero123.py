...
class Zero123(BaseLift3DSystem):
    ...
    def on_fit_start(self) -> None:
        ...
        self.save_image_grid(
            ...
            name="on_fit_start", step=self.true_global_step)
    ...
    def training_substep(self, batch, batch_idx, guidance: str):
        ...
        guidance_eval = guidance == "zero123" and self.cfg.freq.guidance_eval > 0 and self.true_global_step % self.cfg.freq.guidance_eval == 0
        ...
        if guidance == "ref":
            ...
            if self.C(self.cfg.loss.lambda_depth_rel) > 0:
                ...
                set_loss("depth_rel", 1 - self.pearson(valid_pred_depth, valid_gt_depth))

            # normal loss
            if self.C(self.cfg.loss.lambda_normal) > 0:
                valid_gt_normal = 1 - 2 * batch["ref_normal"][gt_mask.squeeze(-1)]  # [B, 3]
                valid_pred_normal = 2 * out["comp_normal"][gt_mask.squeeze(-1)] - 1  # [B, 3]
                set_loss("normal", 1 - F.cosine_similarity(valid_pred_normal, valid_gt_normal).mean())
        elif guidance == "zero123":
            # zero123
            guidance_out = self.guidance(out["comp_rgb"], **batch, rgb_as_latents=False, guidance_eval=guidance_eval)
            # claforte: TODO: rename the loss_terms keys
            set_loss("sds", guidance_out["loss_sds"])

        if self.C(self.cfg.loss.lambda_normal_smooth) > 0:
            if "comp_normal" not in out:
                raise ValueError("comp_normal is required for 2D normal smooth loss, no comp_normal is found in the output.")
        ...
        if self.C(self.cfg.loss.lambda_3d_normal_smooth) > 0:
            ...
            if "normal_perturb" not in out:
                raise ValueError("normal_perturb is required for normal smooth loss, no normal_perturb is found in the output.")
            normals = out["normal"]
        ...
        if not self.cfg.refinement:
            if self.C(self.cfg.loss.lambda_orient) > 0:
                ...
                set_loss(
                    "orient",
                    (out["weights"].detach() * dot(out["normal"], out["t_dirs"]).clamp_min(0.0) ** 2).sum() / (out["opacity"] > 0).sum())
        ...
        if guidance_eval:
            self.guidance_evaluation_save(
                out["comp_rgb"].detach()[: guidance_out["eval"]["bs"]], guidance_out["eval"])
